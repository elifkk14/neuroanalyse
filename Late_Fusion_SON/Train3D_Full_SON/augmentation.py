...
class MRIAugmentation:
    ...
    Intensity transforms are applied only to channel 0 and then masked so
    background / missing voxels stay at zero while channel 1 remains binary.
    """
    ...
    def __call__(self, sample: dict[str, Any]) -> dict[str, Any]:
        ...
        if tensor.ndim != 4:
            raise ValueError(f"Expected image shape (C, D, H, W), got {tuple(tensor.shape)}")

        if tensor.shape[0] != 2:
            raise ValueError(f"Expected 2-channel input, got {tensor.shape[0]} channels.")
    ...
    def _apply_intensity(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if _is_enabled(blur_config) and _chance(float(blur_config.get("p", 0.0))):
            ...
            blurred = _gaussian_blur_3d(channel, sigma)
            channel = torch.where(mask, blurred, channel)
    ...
    def _random_affine(self, image: torch.Tensor, config: dict[str, Any]) -> torch.Tensor:
        ...
        rotate_degrees = config.get("rotate_degrees", [-4, 4])
        translate_voxels = config.get("translate_voxels", [-2, 2])
        scale_range = config.get("scale", [0.97, 1.03])
        ...
        mask_mode = config.get("mode_mask", "nearest")
