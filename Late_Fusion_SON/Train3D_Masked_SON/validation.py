...
def regression_metrics(
    ...
    metrics: dict[str, float] = {
        ...
        "ACC_plusminus_5": float(np.mean(abs_errors <= 5.0)),
        "ACC_plusminus_10": float(np.mean(abs_errors <= 10.0)),
        "ACC_plusminus_15": float(np.mean(abs_errors <= 15.0)),
        "ACC_plusminus_20": float(np.mean(abs_errors <= 20.0)),
    }

    metrics.update(mae_by_centiloid_bin(y_true, abs_errors, bins))
    metrics.update(metrics_0_to_50(y_true, y_pred))
...
def metrics_0_to_50(
    ...
    mask = (y_true >= 0.0) & (y_true <= 50.0)

    if not np.any(mask):
        return {
            "MAE_0_to_50": float("nan"),
            "ACC_0_to_50_plusminus_5": float("nan"),
            "ACC_0_to_50_plusminus_10": float("nan"),
        }

    abs_errors = np.abs(y_pred[mask] - y_true[mask])

    return {
        "MAE_0_to_50": float(abs_errors.mean()),
        "ACC_0_to_50_plusminus_5": float(np.mean(abs_errors <= 5.0)),
        "ACC_0_to_50_plusminus_10": float(np.mean(abs_errors <= 10.0)),
    }
